...
#https://medium.com/coinmonks/what-is-entropy-and-why-inf
#ormation-gain-is-matter-4e85d46d2f01
...
def get_info_gain(splits):
    ...
    e_all = entropy(dist)
    #print("entropy of all: ",e_all)
    for row in splits:
        split_point = float(row[0])
        counter = row[1]
        #calc entropy for children
        ...
        #print(less_neg_per, less_pos_per)
        #print(more_neg_per, more_pos_per)
        ...
        I = e_all - t1 - t2
        #print(I,e_less,e_more)
        #print(counter['less'],counter['more'])
...
class Node:
    ...
    def get_max_depth(self):
        ...
        rgt = 0
        if(self.__left is not None):
            lft = self.__left.get_max_depth()
        if(self.__right is not None):
            rgt = self.__right.get_max_depth()
        best = max(lft,rgt)
        if(best == 0):
...
if __name__ == "__main__":
    A = read_data("occupancy_A.csv")
    t = init_tree(A)
    t.train_tree()
    print('max_depth: ',t.get_max_depth())
    print('accuracy: ',get_prediction_accuracy(t,A))
    n = t.get_plot_node()
    DotExporter(n).to_dotfile("t1.dot")
